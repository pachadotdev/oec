...
#1: read files for the year t-5

trade_t5 = fread2(paste0(temporal_csv_folder,classification,"_rev",rev,"_",t-5,"_",digits,".csv")) %>% 
  countries_codes() %>% 
  rename(export_val5 = export_val, import_val5 = import_val) %>% 
  select(-c(year,marker))

#2: read files for the year t-1
...
yodp_t0 = trade_t0 %>% 
  ...
  left_join(trade_t5, by = join_by("yodp")) %>% 
