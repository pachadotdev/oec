...
for(t in 1:length(csv_list)) {
  if(!file.exists(paste0(csv_folder,"sitc_rev1_bulk_",years[[t]],".csv")) &
     file.exists(paste0(csv_folder,"type-C_r-ALL_ps-",years[t],"_freq-A_px-S1.csv"))) {
    print(paste("Renaming year",years[[t]]))
    file.rename(paste0(csv_folder,"type-C_r-ALL_ps-",years[t],"_freq-A_px-S1.csv"),paste0(csv_folder,"sitc_rev1_bulk_",years[[t]],".csv"))
  } else {
    print("File exists. Skippping.")
  }
}
