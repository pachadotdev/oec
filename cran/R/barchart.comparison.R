...
#' @param STEP is the distance between the years to compare (e.g if my year is 2010 and the distance is 2, then the function creates a chart comparing 2006, 2008, 2010, 2012 and 2014)
#' @examples
#' barchart.comparison(chl, chn, exports, 6, 2010, 2)
#' @keywords functions

barchart.comparison <- function(ORIGIN, DESTINATION, VARIABLE, CLASSIFICATION, YEAR, STEP) {
  ...
  if(code_lenght == 4 | code_lenght == 6) {
    ...
    if(!file.exists(json_file)){
      ...
      ### html ###
      OUTPUT = INPUT
      html_file <- paste0(OUTPUT, "_barchart_", VARIABLE, ".html")
    ...
    print('Barchart.compare only admits 4 characters codes (SITC) or 6 characters codes (HS92).')
  }
}
