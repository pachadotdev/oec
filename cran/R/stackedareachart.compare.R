#' Creates a stacked area chart to compare two years
#' @export
#' @return Creates an \code{HTML} file with a stacked area chart visualization that compares two given years.
...
#' @examples
#' stackedareaplot.compare(chl, chn, 6, 2010, 2014)
#' @keywords functions

stackedareaplot.compare <- function(ORIGIN, DESTINATION, CLASSIFICATION, YEAR1, YEAR2) {
  ...
  if(VARIABLE == "exports") {
    if(code_lenght == 4 | code_lenght == 6) {
      ...
      if(!file.exists(json_file)){
        ...
        ### html ###
        OUTPUT = INPUT
        html_file <- paste0(OUTPUT, "_stackedareaplot_exports", ".html")
    ...
    print('Stackedareaplot.compare only admits 4 characters codes (SITC) or 6 characters codes (HS92). This method only allows "exports".')
  }
}
